...
void WritetoFile(STUDENT stu[],int sum_stu[],int x,int cl);
void ReadfromFile(STUDENT stu[],int sum_stu[],int *x,int *cl);
void PrintfromFile(STUDENT stu[],int sum_stu[],int x,int cl);
int main()
{
    ...
    do{
        ...
        case 4:
            {
                ...
                {
                    {
                        printf("%10ld%8s%4d",stu[p].number,stu[p].name,sum_stu[p]);
                    for(int q=0;q<cl;q++)
                    {
        ...
        case 12:
            {
                WritetoFile(stu,sum_stu,x,cl);
                 break;
            }
        case 13:
            {
                ReadfromFile(stu,sum_stu,&x,&cl);
                PrintfromFile(stu,sum_stu,x,cl);
                break;
            }
        case 0:
            break;
...
void WritetoFile(STUDENT stu[],int sum_stu[],int x,int cl)
{
    ...
    for(p=0;p<x;p++)
    {
        fprintf(fp,"%10ld",stu[p].number);
        fprintf(fp,"%8s",stu[p].name);
        fprintf(fp,"%4d",sum_stu[p]);
        for(q=0;q<cl;q++)
        {
            fprintf(fp,"%4d",stu[p].score[q]);
        }
        fprintf(fp,"\n");
    }
    fclose(fp);
...
void ReadfromFile(STUDENT stu[],int sum_stu[],int *x,int *cl)
{
    ...
    for(p=0;p<*x;p++)
    {
        fscanf(fp,"%10ld",&stu[p].number);
        fscanf(fp,"%8s",stu[p].name);
        fscanf(fp,"%4d",&sum_stu[p]);
        for(q=0;q<*cl;q++)
        {
            fscanf(fp,"%4d",&stu[p].score[q]);
        }
    }
    fclose(fp);
}
